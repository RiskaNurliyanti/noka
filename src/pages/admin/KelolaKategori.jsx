...
export default function KelolaKategori() {
  ...
  async function tambah(e) {
    ...
    if (!nama.trim()) return
    try {
      await api.post('/admin/kategori', { nama })
      setNama('')
  ...
  async function simpanEdit(id) {
    try {
      await api.put(`/admin/kategori/${id}`, { nama: editNama })
      setEditId(null)
  ...
  async function hapus(id, nama) {
    const yakin = await confirmAsync(`Hapus kategori produk "${nama}"?`)
    if (!yakin) return
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-xl font-bold mb-1">🏷️ Kelola kategori produk</h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Contoh: Makanan, Minuman, Snack, Dessert, Sayur, Buah.</p>

      <form onSubmit={tambah} className="flex gap-2 mb-4">
        <input value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Nama kategori produk"
      ...
      {loading ? (
        ...
        <EmptyState
          icon="🏷️"
          title={cari ? 'Tidak ada kategori yang cocok' : 'Belum ada kategori produk'}
              ...
              {editId === k.id ? (
                <>
                  <input value={editNama} onChange={(e) => setEditNama(e.target.value)}
                    className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-sm mr-2" />
